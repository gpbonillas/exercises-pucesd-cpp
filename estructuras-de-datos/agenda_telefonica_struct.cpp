...
void guardar()
{
  ...
  do
  {
    ...
    gotoxy(1,4);printf("Ingrese edad de %s : ",cont.nombre);cin>>cont.edad;
...
void ver()
{
  ...
  gotoxy(5,3);printf("NOMBRES\t\tEDAD\tCELULAR");
  fread(&cont,sizeof(guiatel),1,fic);
  while(feof(fic)==0)
  {
    gotoxy(5,y);printf("%s\t\t%d\t%s",cont.nombre,cont.edad,cont.cel);
    fread(&cont,sizeof(guiatel),1,fic);
    y++;
  }
}
